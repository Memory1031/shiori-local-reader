...
final class ImportBridge: NSObject, FlutterStreamHandler, UIDocumentPickerDelegate {
  ...
  private let worker = DispatchQueue(label: "dev.shiori.localreader.import", qos: .userInitiated)
  ...
  init(messenger: FlutterBinaryMessenger, host: UIViewController) {
    super.init()
    self.host = host
    FlutterMethodChannel(name: "dev.shiori.localreader/import", binaryMessenger: messenger)
      .setMethodCallHandler { [weak self] call, result in
        self?.handle(call, result)
      }
    FlutterEventChannel(name: "dev.shiori.localreader/import_events", binaryMessenger: messenger)
