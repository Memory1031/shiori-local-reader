...
$sourcePath = Join-Path $projectRoot 'assets/branding/shiori-icon.png'
...
try {
    ...
    Copy-Item $sourcePath (Join-Path $projectRoot 'android/app/src/main/res/drawable-nodpi/shiori_launch_logo.png') -Force
    Copy-Item $sourcePath (Join-Path $projectRoot 'ios/Runner/Assets.xcassets/ShioriLaunchLogo.imageset/shiori-logo.png') -Force
    Write-Output 'Updated Android and iOS app icons from assets/branding/shiori-icon.png.'
} finally {
    $sourceImage.Dispose()
}
