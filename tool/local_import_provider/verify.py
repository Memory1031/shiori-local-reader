...
def receipt():
    raw = adb('shell', 'run-as', 'dev.shiori.localreader', 'cat',
...
def launch():
    adb('shell', 'am', 'start', '-n', 'dev.shiori.localreader/.MainActivity')
...
if args.files_only:
    for suffix, content in [('txt', b'Offline Files app fixture.'), ('epub', b'PK\x03\x04intake fixture')]:
        ...
        assert adb('exec-out', 'run-as', 'dev.shiori.localreader', 'cat',
...
for index, (mode, suffix) in enumerate([('view', 'txt'), ('send', 'epub'), ('view', 'epub'), ('send', 'txt')]):
    ...
    if index == 0:
        adb('shell', 'am', 'force-stop', 'dev.shiori.localreader')
    send(mode, name)
    original = wait(receipt)
    assert original['name'] == name
    payload = adb('exec-out', 'run-as', 'dev.shiori.localreader', 'cat', 'no_backup/import-inbox/pending/payload')
