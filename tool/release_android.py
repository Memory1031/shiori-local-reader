...
def verify_metadata(badging, certs, expected_version, expected_build, expected_certificate):
    ...
    if not package or package.groups() != ("dev.shiori.localreader", expected_build, expected_version):
...
def verify_apk(root, tag):
    ...
    target = output / f"shiori-local-reader-{tag}-android.apk"
    ...
    metadata = {"tag": tag, "commit": os.environ["GITHUB_SHA"], "versionName": name,
                "versionCode": int(number), "applicationId": "dev.shiori.localreader",
