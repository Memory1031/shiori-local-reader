...
非 spine XHTML 辅助文档独立保存；普通链接仍使用菜单，WebView 导航关闭。普通链接锚点为语义块级，隐藏目标不揭示；旧 manifest 不自动升级。
...
| 辅助链接 | 普通链接通过“本章链接”菜单打开包内辅助页；脚注支持原生数字上标及底部面板；linear=no 不参与连续阅读；WebView 导航关闭 | epub_links、epub_footnotes、reparse；widget local_links、footnotes |
