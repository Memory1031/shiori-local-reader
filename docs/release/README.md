...
| [CI](../ci.md) 与 Android 发布工作流 | 签名配置、触发规则和上传方式 |
...
第一条只预览；第二条用于已获授权的正式发布。要求工作区干净、develop 与 origin/develop 一致、标签不存在、包内与扩展版本一致。脚本优先快进 master，创建附注标签并原子推送 master / tag，成功后回到 develop；冲突需人工处理。

master 仅是发布中间分支。tag 触发 Android 签名 APK 构建与上传，不等待日常 CI，也不重复 UT / analyze。CI 绿色不能替代本地测试记录。

GitHub Release 优先读取 `docs/release/notes/<tag>.md`，缺失时自动生成；已有 Release 重跑只更新附件，手动编辑过的正文需在 GitHub 单独更新。

## 3. 验收

- Android：核对 Actions、APK、SHA256SUMS、release-info、版本与签名，验证覆盖安装保留数据。
- iOS：无 CI 发布链路，使用本地开发者账户签名；如需分发，手动构建安装并验证启动、导入、阅读与分享扩展。
