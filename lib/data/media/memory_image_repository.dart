...
/// Loads a caller-supplied SourceMedia resolver; used by development
/// scenarios and probes, not by the local book repository.

AppFailure _mediaFailure(Operation operation, FailureKind kind) => AppFailure(
  kind: kind,
  operation: operation,
  ...
  Future<Result<LoadResult<MediaLease>>> load(
    ...
    if (flight == null) {
      if (_flights.length >= 20) {
        return Future.value(
          Failure(
            _mediaFailure(Operation.media, FailureKind.sourceUnavailable),
          ),
        );
      }
      flight = _Flight(ref);
      ...
      owned.timer = Timer(
        deadline,
        () =>
            _abort(owned, _mediaFailure(Operation.media, FailureKind.timeout)),
      );
    }
    final owner = flight;
    ...
    Future<Result<MemoryMedia>> read() async {
      try {
        ...
        if (body.info.format == MediaFormat.unknown) {
          return Failure(
            _mediaFailure(Operation.media, FailureKind.unsupported),
          );
        }
        if ((body.info.byteLength ?? 0) > maxBytes) {
          return Failure(_mediaFailure(Operation.media, FailureKind.tooLarge));
        }
        ...
        flight.stream = body.chunks.listen(
            ...
            switch (chunk) {
              ...
              case Success(:final value):
                if (bytes.length + value.length > maxBytes) {
                    ...
                    Failure(
                      _mediaFailure(Operation.media, FailureKind.tooLarge),
                    ),
                  );
                } else {
          ...
          onError: (Object _) {
            if (!done.isCompleted) {
                ...
                Failure(_mediaFailure(Operation.media, FailureKind.network)),
              );
            }
          },
          onDone: () {
            ...
            if (data.isEmpty ||
                ...
                Failure(_mediaFailure(Operation.media, FailureKind.parse)),
              );
            } else {
              done.complete(
                Success(
        ...
        return Failure(_mediaFailure(Operation.media, FailureKind.network));
      }
    }
