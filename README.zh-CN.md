...
使用合成书籍体验书架与阅读器，可运行开发入口：

```sh
fvm flutter run --target lib/main_dev.dart
```

可用场景与控制项见[离线开发入口与样本](docs/development.md#离线开发入口与样本)。
...
项目代码与品牌图片采用 [Apache-2.0](LICENSE)，第三方组件保留各自许可证。品牌由书签图标与 Shiori 字标组成，来源见[素材说明](assets/branding/README.md)；依赖许可清单见[发布文档](docs/release/dependencies.md)。
