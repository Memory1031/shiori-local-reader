...
void main() {
  ...
  Future<void> open() async {
    ...
    novels = LocalReadingRepository(local: store);
    images = LocalImageRepository(local: store);
  }
  ...
  test(
    'keys outside the local source fail explicitly and never block reads',
    () async {
      final record = ok(await add(LocalBookFormat.txt));
      final local = record.content.detail.summary.key;
      final legacy = NovelKey(
        sourceId: SourceId('lightnovel'),
        novelId: 'legacy-online-entry',
      );
      for (final mode in ReadMode.values) {
        expect(
          await novels.loadDetail(legacy, mode: mode, cancellation: token()),
          isA<Failure<LoadResult<NovelDetail>>>(),
        );
        expect(
          await novels.loadCatalog(legacy, mode: mode, cancellation: token()),
          isA<Failure<LoadResult<Catalog>>>(),
        );
        expect(
          await novels.loadChapter(
            ChapterKey(novelKey: legacy, chapterId: 'c'),
            mode: mode,
            cancellation: token(),
          ),
          isA<Failure<LoadResult<ChapterContent>>>(),
        );
      }
      expect(
        await novels.search(
        ...
        isA<Failure<SearchPage>>(),
      );
      expect(
        await images.load(
          MediaRef(sourceId: SourceId('lightnovel'), mediaId: 'legacy-image'),
          mode: ReadMode.cacheOnly,
          cancellation: token(),
        ),
        isA<Failure<LoadResult<MediaLease>>>(),
      );
      // The legacy entries do not disturb local reading.
      expect(
        ok(
          await novels.loadDetail(
            local,
            mode: ReadMode.cacheOnly,
            cancellation: token(),
          ),
