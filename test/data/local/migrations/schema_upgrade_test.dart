...
void seed(File file, int version) {
  final snapshot =
      jsonDecode(
            File(
              'lib/data/local/database/schemas/user/drift_schema_v$version.json',
            ).readAsStringSync(),
          )
...
void main() {
  ...
  test('migration failure rolls back tables and schema version', () async {
    final file = File('${root.path}/users.sqlite');
    seed(file, 1);
    final before = rows(file);
    final db = FaultUsers(NativeDatabase(file));
    await expectLater(db.customSelect('SELECT 1').get(), throwsA(anything));
    await db.close();
    expect(rows(file), before);
  ...
  test('corrupt file and owned book bytes survive failed opening', () async {
